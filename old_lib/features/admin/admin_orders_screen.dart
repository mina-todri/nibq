...
class _OrderTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isDark ? AppColors.surfRaised : AppColors.surfRaisedLight,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(color: isDark ? AppColors.borderSubtle : AppColors
            .borderSubtleLight),
      ),
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text('طلب #$displayId', style: AppTextStyles.bodyMd.copyWith(
                  fontWeight: FontWeight.bold)),
              _StatusChip(status: order.status),
            ],
          ),
          const Divider(height: 24),
          Text('العميل: ${order.userName}', style: AppTextStyles.bodySm),
          Text('الهاتف: ${order.address.phone}',
              style: AppTextStyles.bodySm.copyWith(color: AppColors.textSecondary)),
          Text('الإجمالي: ${AppConstants.formatEGP(order.total)}',
              style: AppTextStyles.bodyMd.copyWith(color: AppColors.gold400)),
          const SizedBox(height: 12),
          Text('العنوان: ${order.address.district} - ${order.address
              .neighborhood}',
              style: AppTextStyles.bodySm.copyWith(
                  color: AppColors.textSecondary)),
          Text('الدفع: ${order.paymentMethod == 'cash' ? 'كاش' : 'بطاقة'}',
              style: AppTextStyles.bodySm.copyWith(
                  color: AppColors.textSecondary)),
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton(
                ...
                child: const Text(
                    'تحديث الحالة', style: TextStyle(color: Colors.white)),
              ),
            ],
          ),
        ],
  ...
  void _showOrderDetails(BuildContext context, WidgetRef ref,
    ...
    final isDark = Theme
        .of(context)
        .brightness == Brightness.dark;
    final dateStr = DateFormat('yyyy/MM/dd hh:mm a', 'ar').format(
        order.createdAt);
    final displayId = order.id;
            ...
            child: Column(
              ...
              children: [
                ...
                Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('تفاصيل الطلب #$displayId',
                        style: AppTextStyles.displayLg),
                    Text(dateStr, style: AppTextStyles.bodySm.copyWith(
                ...
                // Customer Section
                Text('بيانات العميل', style: AppTextStyles.labelMd.copyWith(
                    color: AppColors.gold400)),
                const SizedBox(height: 8),
                Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    color: isDark ? AppColors.surfBase : AppColors
                        .surfOverlayLight,
                    borderRadius: BorderRadius.circular(12),
                  ...
                  child: Column(
                    children: [
                      ...
                      Row(
                        children: [
                          const Icon(Icons.phone_enabled, size: 20,
                              color: AppColors.success),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Text(order.address.phone,
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      Row(
                        children: [
                          const Icon(Icons.payment, size: 20, color: AppColors.gold400),
                          const SizedBox(width: 12),
                          Text(
                            'الدفع: ${order.paymentMethod == 'cash' ? 'كاش' : 'بطاقة'}',
                            style: AppTextStyles.bodyMd,
                          ),
                        ],
                      ),
                      if (order.note != null && order.note!.isNotEmpty) ...[
                        ...
                        Row(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Icon(Icons.note, size: 20, color: AppColors.gold400),
                            const SizedBox(width: 12),
                            Expanded(
                              child: Text(
                                'ملاحظة: ${order.note}',
                      ...
                      Row(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Icon(Icons.location_on, size: 20, color: AppColors.gold400),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Text(
                ...
                // Items List
                Text('المنتجات', style: AppTextStyles.labelMd.copyWith(
                    color: AppColors.gold400)),
                Expanded(
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        ...
                        subtitle: Text(item.formattedVariant),
                        trailing: Text(
                          AppConstants.formatEGP(item.price * item.quantity),
                          style: AppTextStyles.bodyMd.copyWith(color: AppColors.gold300),),
                      );
                    },
                  ),
                ),
                ...
                // Financial Summary
                _summaryRow(
                    'المجموع الفرعي', AppConstants.formatEGP(order.subtotal)),
                if (order.discount > 0)
                  _summaryRow('خصم إضافي', '- ${AppConstants.formatEGP(order.discount)}', color: AppColors.danger),

                _summaryRow('التوصيل', order.delivery == 0 ? 'مجاناً' : AppConstants.formatEGP(order.delivery)),
                const SizedBox(height: 8),
                _summaryRow(
                    'الإجمالي النهائي', AppConstants.formatEGP(order.total),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value,
              style: AppTextStyles.bodyMd.copyWith(
                color: color ?? (isBold ? AppColors.gold400 : null),
                fontWeight: isBold ? FontWeight.bold : null,
                fontSize: fontSize,
              )),
        ],
  ...
  void _copyForDelivery(BuildContext context, OrderModel order) {
    final text = "الاسم: ${order.userName} | "
        ...
        "العنوان: ${order.address.fullAddress} | "
